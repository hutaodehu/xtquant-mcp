...
from xtqmt_mcp.trade_gateway.session_manager import SessionWarmError
...
class _TimeoutingProbeSessionManager(_FakeSessionManager):
...
class TradeGatewayServerTests(unittest.TestCase):
    ...
    def test_probe_connection_timeout_keeps_session_plan_payload_in_error_envelope(self) -> None:
        with _workspace_tempdir() as tmp:
            root = Path(tmp)
            config = TradeGatewayConfig(
                identity=ServiceIdentity(server_name="xtqmtTradeGateway", server_version="test"),
            ...
            server = TradeGatewayServer(config, login_handler=lambda cfg: _FakeLoginResult())
            server._session_manager = _TimeoutingProbeSessionManager()
            ...
            payload = server.dispatch(
                {
                    ...
                    "id": 42,
                    "method": "tools/call",
                    "params": {"name": "probe.connection", "arguments": {}},
                }
            )

            structured = payload["result"]["structuredContent"]
            self.assertFalse(structured["ok"])
            self.assertEqual(structured["error"]["code"], "session_start_timeout")
            self.assertEqual(structured["data"]["reason"], "session_start_timeout")
            self.assertEqual(structured["data"]["session_plan_version"], "v1:2111,2100,2101")
            self.assertFalse(structured["data"]["same_plan_verdict"])
            self.assertFalse(structured["data"]["probe_complete_verdict"])
            self.assertFalse(structured["data"]["fresh_connect_verified"])
            self.assertFalse(structured["data"]["write_authority_ready"])
            self.assertFalse(structured["data"]["write_permission_ready"])
            self.assertEqual(structured["data"]["write_permission_block_reason"], "session_start_timeout")
            self.assertEqual(structured["data"]["write_session_alignment"]["resolved_session_id"], "2111")
            self.assertEqual(structured["data"]["write_session_alignment"]["observed_probe_session_id"], "")
            self.assertEqual(
                structured["data"]["write_session_alignment"]["effective_session_plan"],
                [2111, 2100, 2101],
            )
            self.assertEqual(structured["data"]["session_resolution"]["resolved_session_id"], 2111)
            self.assertEqual(structured["data"]["session_resolution"]["effective_session_plan"], [2111, 2100, 2101])
            self.assertEqual(structured["data"]["effective_session_resolution"]["resolved_session_id"], 2111)
