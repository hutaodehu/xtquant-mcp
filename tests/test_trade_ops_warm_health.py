...
class _ProbeOnlyWarmHealthShadow:
    def __init__(self) -> None:
        self.calls = 0
        self.probe_calls = 0
    ...
    def get_orders(self) -> list[dict[str, object]]:
        self.calls += 1
        raise AssertionError("warm health probe-only path should not query shadow orders")
...
class TradeOpsWarmHealthTests(unittest.TestCase):
    ...
    def test_warm_health_orders_list_can_use_probe_only_shadow_smoke_without_querying_orders(self) -> None:
        work_root = ROOT / "instance" / "test_tmp" / "tg006_warm_health_probe_only_smoke"
        shutil.rmtree(work_root, ignore_errors=True)
        work_root.mkdir(parents=True, exist_ok=True)
        cfg = TradeOpsConfig(
            account_id="ACC001",
        ...
        shadow = _ProbeOnlyWarmHealthShadow()
        service = TradeOpsService(
            cfg,
            market_data_provider=object(),
            shadow_adapter=shadow,
        ...
        try:
            result = service.warm_health_orders_list()
            self.assertTrue(result.ok)
            self.assertEqual(shadow.probe_calls, 1)
            self.assertEqual(shadow.calls, 0)
            self.assertEqual(result.payload["source"], "xttrader_shadow")
            self.assertEqual(result.payload["read_scope"], "warm_health_only")
            self.assertEqual(result.payload["truth_scope"], "shadow_warm_health")
            self.assertEqual(result.payload["count"], 0)
            self.assertTrue(result.payload["warm_health_probe_only"])
            self.assertEqual(result.payload["session_id"], "2100")
            self.assertEqual(result.payload["shadow_probe"]["positions_rows"], 2)
            self.assertEqual(result.payload["shadow_probe"]["asset_rows"], 1)
        finally:
            service.close()
            shutil.rmtree(work_root, ignore_errors=True)
