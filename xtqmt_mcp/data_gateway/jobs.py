...
ACTIVE_QUEUE_JOB_STATUSES = {"pending", "running"}
...
class DownloadJobRequest:
    ...
    future_day_calendar: tuple[str, ...] = ()

    def as_dict(self) -> dict[str, Any]:
        payload = asdict(self)
        payload["codes"] = list(self.codes)
        payload["periods"] = list(self.periods)
        payload["future_day_calendar"] = list(self.future_day_calendar)
        return payload
...
class DownloadJobManager:
    ...
    def _active_job_ids(self) -> list[str]:
        return [job_id for job_id, state in self._jobs.items() if state.status in ACTIVE_QUEUE_JOB_STATUSES]
    ...
    def _run_job(self, job_id: str) -> None:
        ...
        def on_progress(progress: dict[str, Any]) -> bool:
            with self._lock:
                ...
                for key, value in dict(progress or {}).items():
                    if key in sample or key in {"finished", "total", "message"}:
                        continue
        ...
        try:
            ...
            with self._lock:
                ...
                forced_status = str(current.result.get("force_status", "") or "").strip()
                if current.cancel_requested:
                    current.status = "cancelled"
                elif forced_status in FINAL_JOB_STATUSES:
                    current.status = forced_status
                else:
    ...
    def _load_existing_jobs(self) -> None:
        for path in sorted(self._jobs_root.glob("*.json")):
            ...
            state = DownloadJobState(
                ...
                request=DownloadJobRequest(
                    job_kind=str(request_payload.get("job_kind", "download") or "download"),
                    ...
                    target_date=str(request_payload.get("target_date", "") or ""),
                    periods=tuple(str(item).strip() for item in request_payload.get("periods", []) if str(item).strip()),
                    symbols_scope=str(request_payload.get("symbols_scope", "") or ""),
                    local_qlib_dir=str(request_payload.get("local_qlib_dir", "") or ""),
                    wsl_qlib_dir=str(request_payload.get("wsl_qlib_dir", "") or ""),
                    adjusted_mode=str(request_payload.get("adjusted_mode", "raw_with_factor") or "raw_with_factor"),
                ),
