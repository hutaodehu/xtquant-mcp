...
class DataGatewayRuntimeConfig:
    ...
    plans_root: str = str(Path(DEFAULT_RUNTIME_PATHS.artifact_root) / "data_plans")
    cache_root: str = str(Path(DEFAULT_RUNTIME_PATHS.artifact_root) / "data_cache")
    metadata_path: str = str(Path(DEFAULT_RUNTIME_PATHS.artifact_root) / "data_cache" / "integrity_state.json")
    windows_qlib_root: str = r"C:\xtquant-mcp-example\qlib_data\xtdata_export_local"
    wsl_qlib_root: str = "/opt/xtquant-mcp-example/qlib_data/xtdata_export_local"
    wsl_distro_name: str = ""
...
def load_data_gateway_config(
    ...
    *,
    ...
    service_cfg = DataGatewayRuntimeConfig(
        ...
        plans_root=str(pick(service_payload, "plans_root", Path(paths_cfg.artifact_root) / "data_plans") or (Path(paths_cfg.artifact_root) / "data_plans")),
        cache_root=str(pick(service_payload, "cache_root", Path(paths_cfg.artifact_root) / "data_cache") or (Path(paths_cfg.artifact_root) / "data_cache")),
        metadata_path=str(pick(service_payload, "metadata_path", Path(paths_cfg.artifact_root) / "data_cache" / "integrity_state.json") or (Path(paths_cfg.artifact_root) / "data_cache" / "integrity_state.json")),
        windows_qlib_root=env_text(active_env, "XTQMT_WINDOWS_QLIB_ROOT") or str(pick(service_payload, "windows_qlib_root", r"C:\xtquant-mcp-example\qlib_data\xtdata_export_local") or r"C:\xtquant-mcp-example\qlib_data\xtdata_export_local"),
        wsl_qlib_root=env_text(active_env, "XTQMT_WSL_QLIB_ROOT") or str(pick(service_payload, "wsl_qlib_root", "/opt/xtquant-mcp-example/qlib_data/xtdata_export_local") or "/opt/xtquant-mcp-example/qlib_data/xtdata_export_local"),
        wsl_distro_name=env_text(active_env, "XTQMT_WSL_DISTRO_NAME", "WSL_DISTRO_NAME") or str(pick(service_payload, "wsl_distro_name", "") or ""),
        stale_job_seconds=coerce_int(pick(service_payload, "stale_job_seconds"), 300),
