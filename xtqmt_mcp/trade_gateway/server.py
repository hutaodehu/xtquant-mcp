...
from xtqmt_mcp.session_resolution import build_session_plan_version, session_resolution_payload
...
from xtqmt_mcp.legacy_ports import legacy_port_fields
...
class TradeGatewayServer:
        ...
        if tool_name == "probe.connection":
            ...
            try:
                ...
                return envelope_from_exception(
                    ...
                    artifacts=base_artifacts,
                    server_ts=server_now.isoformat(timespec="seconds"),
                    data=self._normalize_probe_timeout_payload(dict(exc.payload), error_code=error_code),
                )
    ...
    def _normalize_probe_timeout_payload(self, payload: dict[str, Any], *, error_code: str) -> dict[str, Any]:
        enriched = self._decorate_payload(dict(payload or {}), scope="primary_session")
        session_resolution = session_resolution_payload(enriched.get("session_resolution") or {})
        effective_resolution_raw = enriched.get("effective_session_resolution")
        effective_session_resolution = (
            session_resolution_payload(effective_resolution_raw)
            if isinstance(effective_resolution_raw, dict) and effective_resolution_raw
            else {}
        ) or dict(session_resolution)
        resolved_session_id = str(
            effective_session_resolution.get("resolved_session_id")
            or session_resolution.get("resolved_session_id")
            or enriched.get("session_id")
            or ""
        ).strip()
        resolved_base_session_id = str(
            effective_session_resolution.get("resolved_base_session_id")
            or session_resolution.get("resolved_base_session_id")
            or ""
        ).strip()
        observed_probe_session_id = str(enriched.get("observed_probe_session_id") or "").strip()
        effective_session_plan = list(
            effective_session_resolution.get("effective_session_plan")
            or session_resolution.get("effective_session_plan")
            or []
        )
        session_plan_version = (
            str(enriched.get("session_plan_version") or "").strip()
            or str(effective_session_resolution.get("session_plan_version") or "").strip()
            or str(session_resolution.get("session_plan_version") or "").strip()
            or build_session_plan_version(effective_session_plan)
        )
        if not resolved_session_id:
            same_plan_reason = "resolved_write_session_missing"
        elif not observed_probe_session_id:
            same_plan_reason = "probe_session_missing"
        elif observed_probe_session_id == resolved_session_id:
            same_plan_reason = "same_session"
        else:
            same_plan_reason = "probe_session_differs_from_resolved_write_session"
        write_permission_block_reason = str(
            enriched.get("write_permission_block_reason")
            or enriched.get("reason")
            or error_code
            or "probe_connection_unavailable"
        ).strip()
        enriched["session_resolution"] = session_resolution
        enriched["effective_session_resolution"] = effective_session_resolution
        enriched["session_plan_version"] = session_plan_version
        enriched.setdefault("same_plan_verdict", False)
        enriched.setdefault("probe_complete_verdict", False)
        enriched.setdefault("fresh_connect_verified", False)
        enriched.setdefault("write_authority_ready", False)
        enriched.setdefault("write_permission_ready", False)
        enriched["write_permission_block_reason"] = write_permission_block_reason
        enriched["write_session_alignment"] = {
            "resolved_session_id": resolved_session_id,
            "resolved_base_session_id": resolved_base_session_id,
            "effective_session_plan": effective_session_plan,
            "session_plan_version": session_plan_version,
            "observed_probe_session_id": observed_probe_session_id,
            "observed_probe_session_in_effective_plan": bool(
                observed_probe_session_id and observed_probe_session_id in {str(item) for item in effective_session_plan}
            ),
            "same_session_as_write_path": bool(observed_probe_session_id and observed_probe_session_id == resolved_session_id),
