...
def _extract_tick_payload(payload) -> tuple[
    ...
    if isinstance(payload, np.ndarray):
        ...
        if payload.dtype.names:
            ...
            return last_price, time_val, bid1, bid2, bid3, ask1, ask2, ask3, prev_close
...
def _query_tick_payload(xtdata, code: str, date_str: str):
    data = xtdata.get_market_data(
        field_list=["lastPrice", "lastClose", "open", "time"],
        stock_list=[code],
        period="tick",
        start_time=date_str,
        end_time=date_str,
        ...
        if period == "tick":
            ...
            if not _tick_payload_available(payload):
                try:
                    ...
                    payload = full_tick.get(code) if isinstance(full_tick, dict) else payload
                except Exception:
                    pass
            last_price, ts, bid1, bid2, bid3, ask1, ask2, ask3, prev_close = _extract_tick_payload(payload)
