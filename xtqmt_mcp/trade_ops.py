...
class TradeOpsService:
    ...
    def _configured_session_candidate_tokens(self) -> list[str]:
        plan = self.session_resolution.get("configured_session_candidates") or []
        tokens: list[str] = []
        for item in plan:
            token = str(item or "").strip()
            if token:
    ...
    def _cached_broker_adapter_matches_session_resolution(self, adapter: Any) -> bool:
        ...
        if not expected_session_id:
            return True
        actual_session_id = self._broker_adapter_session_id(adapter)
    ...
    def _shadow_supports_governed_broker_ops(self) -> bool:
        shadow = self.shadow
        if shadow is None:
            return False
        required_methods = (
        ...
        return all(callable(getattr(shadow, name, None)) for name in required_methods)

    def _promote_owner_managed_shadow_as_broker(self) -> bool:
        if not self._shadow_supports_governed_broker_ops():
            return False
        self._broker_adapters[False] = self.shadow
        self._broker_adapters[True] = self.shadow
        self.broker = self.shadow
        return True
    ...
    def _ensure_broker_adapter(self, *, require_write_permission: bool) -> Any | None:
        ...
        if adapter is not None:
            if callable(self._broker_order_adapter_factory) and (
                ...
                return adapter
        if not mode:
            promoted_write_adapter = self._broker_adapters.get(True)
            if promoted_write_adapter is not None:
                if callable(self._broker_order_adapter_factory) and (
                    not self._cached_broker_adapter_matches_session_resolution(promoted_write_adapter)
                ):
                    self._drop_cached_broker_adapter(True, promoted_write_adapter)
                else:
                    self._broker_adapters[False] = promoted_write_adapter
                    self.broker = promoted_write_adapter
                    return promoted_write_adapter
    ...
    def _snapshot_l1(self, code: str) -> L1Snapshot | None:
        ...
        return L1Snapshot(
            ...
            bid4=_safe_float(getattr(event, "bid4", None)),
            bid5=_safe_float(getattr(event, "bid5", None)),
            ask1=_safe_float(event.ask1),
            ask2=_safe_float(getattr(event, "ask2", None)),
            ask3=_safe_float(getattr(event, "ask3", None)),
            ask4=_safe_float(getattr(event, "ask4", None)),
            ask5=_safe_float(getattr(event, "ask5", None)),
                ...
                int(
                    sum(
                        ...
                        for bid, ask in (
                            ...
                            (_safe_float(getattr(event, "bid4", None)), _safe_float(getattr(event, "ask4", None))),
                            (_safe_float(getattr(event, "bid5", None)), _safe_float(getattr(event, "ask5", None))),
                        )
    ...
    def _to_result(self, command: str, result: TradeCommandResult) -> TradeOpsResult:
        ...
        if result.l1_snapshot is not None:
            l1_payload = {
                ...
                "bid4": result.l1_snapshot.bid4,
                "bid5": result.l1_snapshot.bid5,
                "ask1": result.l1_snapshot.ask1,
                "ask2": result.l1_snapshot.ask2,
                "ask3": result.l1_snapshot.ask3,
                "ask4": result.l1_snapshot.ask4,
                "ask5": result.l1_snapshot.ask5,
    ...
    def warm_health_orders_list(self) -> TradeOpsResult:
        ...
        if callable(probe_fn):
            ...
            if isinstance(probe, dict):
                read_mode = str(probe.get("warm_health_read_mode") or "").strip().lower()
                if (
                    read_mode == "probe_only"
                    and bool(probe.get("available", False))
                    and bool(probe.get("reused_session", False))
                    ...
                    return self._result(
                        ...
                        payload={
                            ...
                            "warm_health_probe_only": True,
                            "message": "warm health reused active owner-managed shadow session; skipped shadow orders query",
                            "session_id": str(probe.get("session_id") or ""),
                            "shadow_probe": {
                                "available": bool(probe.get("available", False)),
                                "reused_session": bool(probe.get("reused_session", False)),
                                "positions_rows": _safe_int(probe.get("positions_rows"), 0),
                                "asset_rows": _safe_int(probe.get("asset_rows"), 0),
                                "reason": str(probe.get("reason") or ""),
                            },
                            **self._orders_truth_meta(
                                ...
                                broker_truth_confirmed=False,
                                shadow_fallback_used=False,
                                truth_reason="warm_health_probe_only",
                            ),
                        },
                    )
        payload_rows = self._shadow_order_payload_rows()
    ...
    def _owner_managed_probe_report(self) -> dict[str, Any] | None:
        ...
        configured_xtdata_port = coerce_port(getattr(self.cfg, "xtdata_port", 0), 0)
        xtdata_port_configured = configured_xtdata_port > 0
        vendor_port_ready = bool(
            True
            if not xtdata_port_configured
            else _tcp_port_ready("127.0.0.1", configured_xtdata_port, timeout_ms=300)
        )
        ...
        elif xtdata_port_configured and not vendor_port_ready:
        ...
        precheck = {
            ...
            "xtdata_port_configured": bool(xtdata_port_configured),
            "xtdata_port": int(configured_xtdata_port),
            **legacy_port_fields(configured_xtdata_port),
            "readiness_layers": {
                "read_only": {
        ...
        connection_trace = [
            ...
            },
            {
                ...
                "code": (
                    "ok"
                    if bool(vendor_port_ready) and xtdata_port_configured
                    else "xtdata_port_unconfigured"
                    if bool(vendor_port_ready)
                    else "xtdata_port_not_ready"
                ),
                "message": (
                    f"xtdata ready on 127.0.0.1:{configured_xtdata_port}"
                    if bool(vendor_port_ready) and xtdata_port_configured
                    else "xtdata runtime port is unconfigured; port probe skipped"
                    if bool(vendor_port_ready)
                    else f"xtdata not ready on 127.0.0.1:{configured_xtdata_port}"
                ),
                "latency_ms": None,
                "retry_count": 0,
                "details": {
                    "port": str(configured_xtdata_port),
                    "configured": str(bool(xtdata_port_configured)),
        ...
        if (
            bool(owner_probe_ok)
            and bool(owner_probe_session_id)
            and self._shadow_supports_governed_broker_ops()
            and (owner_probe_session_id in self._configured_session_candidate_tokens())
        ):
            self.realign_session_resolution(
                owner_probe_session_id,
                reason="owner_session_detected",
                event_source="owner_managed_session",
                owner_session_id=owner_session_id,
                observed_probe_session_id=owner_probe_session_id,
                attempted_broker_session_id=owner_probe_session_id,
            )
            write_alignment = self._build_write_session_alignment(owner_probe_session_id)
        ...
        if (
            ...
            if self._promote_owner_managed_shadow_as_broker():
                fresh_connect_attempted = True
                selected_session_id = owner_probe_session_id
                connection_trace.extend(
                    [
                        {
                            "name": f"connect_session_{owner_probe_session_id}",
                            "ok": True,
                            "code": "0",
                            "message": "owner-managed live session promoted for governed broker reuse",
                            "latency_ms": None,
                            "retry_count": 0,
                            "details": {
                                "source": "owner_managed_shadow",
                                "positions_rows": str(probe.get("positions_rows", "")),
                                "asset_rows": str(probe.get("asset_rows", "")),
                            },
                        },
                        {
                            ...
                            "ok": True,
                            "code": "0",
                            "message": "owner-managed live session already subscribed for governed broker reuse",
                            "latency_ms": None,
                            "retry_count": 0,
                            "details": {
                ...
                if isinstance(broker_verify, dict) and bool(broker_verify.get("available", False)):
                    ...
                    if broker_session_id and bool(broker_verify.get("ok", False)):
                        self.realign_session_resolution(
                            broker_session_id,
    ...
    def _price_from_l1(self, snapshot: L1Snapshot, side: Side) -> float | None:
        primary_price = _safe_float(snapshot.ask1) if side == Side.BUY else _safe_float(snapshot.bid1)
        if primary_price is not None and primary_price > 0:
            return primary_price
        fallback_last_price = _safe_float(snapshot.last_price)
        if fallback_last_price is not None and fallback_last_price > 0:
            return fallback_last_price
        return None
