...
from .session_resolution import build_session_plan_version, session_resolution_payload

DEFAULT_REPO_ROOT = Path("C:/xtquant-mcp-example/repo")
DEFAULT_INSTANCE_ROOT = Path("C:/xtquant-mcp-example/instance/prod")
...
def _diag_probe_resolution(diag_probe: dict[str, Any]) -> dict[str, Any]:
    effective_resolution_raw = diag_probe.get("effective_session_resolution")
    ...
    return session_resolution_payload(diag_probe.get("session_resolution") or {})


def _diag_probe_write_session_alignment(diag_probe: dict[str, Any]) -> dict[str, Any]:
    alignment = dict(diag_probe.get("write_session_alignment") or {})
    resolution = _diag_probe_resolution(diag_probe)
    effective_session_plan = list(alignment.get("effective_session_plan") or resolution.get("effective_session_plan") or [])
    session_plan_version = (
        str(alignment.get("session_plan_version") or "").strip()
        or str(diag_probe.get("session_plan_version") or "").strip()
        or str(resolution.get("session_plan_version") or "").strip()
        or build_session_plan_version(effective_session_plan)
    )
    resolved_session_id = str(
        alignment.get("resolved_session_id") or resolution.get("resolved_session_id") or diag_probe.get("session_id") or ""
    ).strip()
    observed_probe_session_id = str(
        diag_probe.get("observed_probe_session_id")
        or alignment.get("observed_probe_session_id")
        or resolution.get("observed_probe_session_id")
        or ""
    ).strip()
    if not resolved_session_id:
        ...
        same_plan_reason = str(alignment.get("same_plan_reason") or "").strip() or "write_session_mismatch"
    enriched = dict(alignment)
    enriched["resolved_session_id"] = resolved_session_id
    if effective_session_plan:
        enriched["effective_session_plan"] = effective_session_plan
    enriched["session_plan_version"] = session_plan_version
    enriched["observed_probe_session_id"] = observed_probe_session_id
    enriched.setdefault("same_plan_reason", same_plan_reason)
    return enriched
...
def build_trade_write_authority_report(
    *,
    ...
    write_session_alignment = _diag_probe_write_session_alignment(diag_probe)
    same_plan_reason = str((write_session_alignment.get("same_plan_reason") or "")).strip()
    diag_blocking_reason = str(diag_probe.get("write_permission_block_reason") or diag_probe.get("reason") or "").strip()
    session_plan_version = str(write_session_alignment.get("session_plan_version") or "").strip() or build_session_plan_version(
        diag_probe.get("effective_session_plan")
    )
    ...
    resolved_session_id = str(
        (write_session_alignment.get("resolved_session_id") or diag_probe.get("session_id") or "")
    ).strip()
    observed_probe_session_id = str(
        diag_probe.get("observed_probe_session_id")
        or (write_session_alignment.get("observed_probe_session_id") or "")
    ).strip()
