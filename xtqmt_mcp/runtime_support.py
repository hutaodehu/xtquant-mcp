...
def port_ready(host: str = "127.0.0.1", port: int = 58610, timeout_ms: int = 300) -> bool:
...
def ensure_miniqmt_awake(
    *,
    ...
    report = {
        ...
        "xtdata_port_ready_before": port_ready(host=host, port=port),
    ...
    while time.time() < deadline:
        if port_ready(host=host, port=port):
            report["xtdata_port_ready_after"] = True
            report["status"] = "xtdata_port_ready"
            report["ok"] = True
            return report
        time.sleep(1.0)
    report["xtdata_port_ready_after"] = port_ready(host=host, port=port)
    ...
    if not report["ok"]:
        report["error"] = "xtdata_port_not_ready"
    return report
