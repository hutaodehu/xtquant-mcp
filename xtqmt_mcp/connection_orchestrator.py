...
from .legacy_ports import coerce_port, legacy_port_fields
...
def run_connection_orchestrator(cfg: ConnectionOrchestratorConfig) -> ConnectionTraceReport:
    ...
    resolved_port = coerce_port(cfg.port_num, 0)
    port_ready = False
    deadline = time.time() + max(1, int(cfg.wake_wait_seconds))
    while resolved_port > 0 and time.time() < deadline:
        port_ready = _tcp_port_ready(cfg.port_host, resolved_port, timeout_ms=300)
        if port_ready:
            break
        time.sleep(1.0)
    precheck["xtdata_port_ready"] = port_ready
    precheck.update(legacy_port_fields(resolved_port))
    wait_code = "ok" if port_ready else "xtdata_port_not_ready"
    if resolved_port <= 0:
        wait_code = "xtdata_port_unconfigured"
    stages.append(
        ConnectionStageResult(
            ...
            code=wait_code,
            message=(
                f"xtdata ready on {cfg.port_host}:{resolved_port}"
                if port_ready
                else f"xtdata not ready on {cfg.port_host}:{resolved_port or 'unconfigured'}"
            ),
