...
def _is_login_action_button(value: str) -> bool:
    normalized = _compact_match_text(value)
    if not normalized:
        return False
    return normalized in {_compact_match_text(keyword) for keyword in LOGIN_BUTTON_KEYWORDS if keyword}
...
def _classify_window_descriptor(descriptor: WindowDescriptor, *, port_ready: bool = False) -> WindowClassification:
    ...
    for index, button in enumerate(buttons):
        if _is_login_action_button(button.name):
    ...
    login_candidate = bool(edits) and (
        ...
        or (password_label_detected and (submit_button_index is not None or _contains_any(text_blob, REMEMBER_KEYWORDS + AUTO_LOGIN_KEYWORDS)))
        or (_contains_any(descriptor.title, LOGIN_TITLE_KEYWORDS) and login_signal and not main_signal)
...
class WindowsDesktopHarness:
    ...
    def port_ready(self, host: str = "127.0.0.1", port: int = 0, timeout_ms: int = 300) -> bool:
    ...
    def observe(self, *, process_id: int | None = None, port_host: str = "127.0.0.1", port_num: int = 0) -> DesktopObservation:
        resolved_port = coerce_port(port_num, 0)
        port_ready = self.port_ready(host=port_host, port=resolved_port)
        ...
        evidence = {
            ...
            "port_host": str(port_host or "127.0.0.1"),
            "port_num": int(resolved_port),
            "blocking_reason": "",
